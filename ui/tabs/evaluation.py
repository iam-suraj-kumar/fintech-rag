...
def render_single_run() -> None:
    ...
    if st.button("Run evaluation"):
        ...
        cols[2].metric("Faithfulness", f"{mean_faith:.1f}/5" if mean_faith is not None else "-")
        mean_correct = _mean([r["correctness"] for r in findable])
        cols[3].metric("Correctness", f"{mean_correct:.1f}/5" if mean_correct is not None else "-")
...
def render() -> None:
    st.caption(
        ...
        "MRR) are deterministic; faithfulness and correctness are scored by an LLM judge "
